...
public class NoticeOfDepartureCommand {
    ...
    public void view(SlashCommandInteractionEvent event) {
        ...
        String currentDate = LocalDate.now().format(DateTimeFormatter.ofPattern(config.settings().noticeOfDeparture().dateFormatting()));
        String endDate = new NoticeOfDepartureTable().retrieveEndDate(user.getId());

        var embed = new EmbedBuilder()
                .setTitle(new ColorTool().parse(translation.noticeOfDeparture().embedNoticeViewTitle()
                        .replace("%user%", user.getName())))
                .setDescription(new ColorTool().parse(translation.noticeOfDeparture().embedNoticeViewBody()
    ...
    public void revoke(SlashCommandInteractionEvent event) {
        ...
        String endDate = new NoticeOfDepartureTable().retrieveEndDate(user.getId());
        event.replyModal(new NoticeOfDepartureTemplateModals(config, translation).reasonActionModal(ActionId.REVOKING, user.getId(), endDate)).queue();
    }
