...
public class SettingsCommand {
    ...
    public void pasteSettingsMenu(SlashCommandInteractionEvent event) {
        ...
        if (checkCedmod())
            cedmod = translation.settings().textCedmodOnline();

        ColorTool colorTool = new ColorTool();

        var builder = new EmbedBuilder()
                .setAuthor(event.getGuild() != null ? event.getGuild().getName() : null, null,
                ...
                .addField(
                        colorTool.parse(
                                translation.settings().embedSettingsFieldLanguageTitle().trim()),
                ...
                .addField(
                        colorTool.parse(
                                translation.settings().embedSettingsFieldGuildTitle().trim()),
                        colorTool.parse(translation.settings().embedSettingsFieldGuildValue()
                                .replace("%isAvailable%",
                                        String.valueOf(event.getGuild() != null && event.getJDA()
                                                .isUnavailable(event.getGuild().getIdLong())))
                                .trim()),
                        true)
                .addField(
                        colorTool.parse(
                                translation.settings().embedSettingsFieldDatabaseTitle().trim()),
                        colorTool.parse(translation.settings().embedSettingsFieldDatabaseValue()
                                .replace("%state%", translation.settings().textDatabaseOnline())
                                .trim()),
                        true)
                .addField(
                        colorTool.parse(
                                translation.settings().embedSettingsFieldCedmodTitle().trim()),
                        cedmod, true)
                .addField(
                        colorTool.parse(
                                translation.settings().embedSettingsFieldVersionTitle().trim()),
                        colorTool.parse(translation.settings().embedSettingsFieldVersionValue()
                                .replace("%version%", version()).trim()),
                        true)
                .addField(
                        colorTool.parse(
                                translation.settings().embedSettingsFieldBuildTitle().trim()),
                        colorTool.parse(translation.settings().embedSettingsFieldBuildValue()
                                .replace("%build%", GlobalVariables.upstreamVersion).trim()),
                        true)
                .addField(
                        colorTool.parse(
                                translation.settings().embedSettingsFieldGatewayTitle().trim()),
                        colorTool.parse(translation.settings().embedSettingsFieldGatewayValue()
                                .replace("%time%", String.valueOf(event.getJDA().getGatewayPing()))
                                .trim()),
                        true)
                .addField("\u200E \n\u200E", "\u200E", true).addField(
                        colorTool
                                .parse(translation.settings().embedSettingsFieldRestTitle().trim()),
                        colorTool.parse(translation.settings().embedSettingsFieldRestValue()
                                .replace("%time%",
                                        String.valueOf(event.getJDA().getRestPing().complete()))
                                .trim()),
                        true);
                        ...
                        ActionRow
                                .of(Button
                                        .secondary("start_page",
                                                colorTool.parse(translation.buttons().textSettingsStart()))
                                        .asDisabled(),
                                        Button.primary("configure_page",
                                                colorTool.parse(translation.buttons().textSettingsConfigure())),
                                        Button.primary("current_settings",
                                                colorTool.parse(translation.buttons().textSettingsCurrent())),
                                        Button.primary("bot_info",
                                                colorTool.parse(translation.buttons()
                                                        .textSettingsInformation())),
                                        Button.link("https://github.com/Vxrpenter/SCPToolsBot",
                                                colorTool.parse(
                                                        translation.buttons().textSettingsNews()))
    ...
    private boolean checkCedmod() {
        if (config.settings().cedmod().active()) {
            try {
                new Cedmod(config.settings().cedmod().instance(), config.settings().cedmod().api(),
                        60, 60).changelogGet();
                return true;
            } catch (Exception e) {
                return false;
    ...
    private String version() {
        ...
        try (var stream = SettingsCommand.class.getResourceAsStream("/dev/vxrp/version.properties")) {
            if (stream == null)
                throw new RuntimeException("Version properties file does not exist");
